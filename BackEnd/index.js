...
async function traerPais(id){
    const res = await sequelize.query('SELECT * FROM paises WHERE paises.id = ?',{replacements:[id],type: sequelize.QueryTypes.SELECT});
    return res;
}

async function traerCiudad(id){
  const res = await sequelize.query('SELECT * FROM ciudades WHERE paises.id = ?',{replacements:[id],type: sequelize.QueryTypes.SELECT});
  return res;
}
